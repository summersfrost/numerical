 ...
 * @author Frost
 */
public class InitialBoundsSecant extends javax.swing.JFrame {
    ...
    public InitialBoundsSecant(String terms, BigDecimal[] coefficients, int[] degree) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("INITIAL BOUNDS (Secant Method)");
  ...
  try {
    upper = new BigDecimal(stringUpper);
      ...
      lower = upper.add(new BigDecimal(1));
    //  eval.ComputeFx(upper, coefficients, degree);
    //  eval.ComputeFx(lower,coefficients,degree);
    
  //Check if the inputted upper bound and lower is an integer
  } catch (NumberFormatException e) {
    JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Upper and Lower must be real numbers.", "Error", JOptionPane.ERROR_MESSAGE);
    return;
  }

  // Check that the upper and lower values are not equal
  if (upper.equals(lower)) {
JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Upper and Lower cannot be equal.", "Error", JOptionPane.ERROR_MESSAGE);
  ...
  if (upper.subtract(lower).abs().compareTo(three) > 0) {
    JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Upper and Lower cannot be more than 3 units apart.","Error",JOptionPane.ERROR_MESSAGE);
    return;
  }
  ...
  if (upperFx.multiply(lowerFx).compareTo(new BigDecimal("0")) > 0) {
    JOptionPane.showMessageDialog(InitialBoundsSecant.this, "LOWER AND UPPER F(X) HAVE THE SAME SIGN","Error",JOptionPane.ERROR_MESSAGE);
    return;
  }
  this.upper = upper;
  this.lower = lower;
  ...
  if (marginErrorRadio) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Margin error must be a real number.","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    if (marginErrorValue.compareTo(BigDecimal.ZERO) <= 0 || marginErrorValue.compareTo(BigDecimal.ONE) >= 0) {
      JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Margin error must be between 0 and 1 (exclusive).","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    try {
      ...
      JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Number of iterations must be a positive integer.","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    if (noIter <= 0) {
      JOptionPane.showMessageDialog(InitialBoundsSecant.this, "Number of iterations must be a positive integer.","Error",JOptionPane.ERROR_MESSAGE);
  ...
  CalcSecant secant = new CalcSecant();
  if (modeIterRegula == 1) {
    secant.errorFalsi(terms, coefficients, degree, tempLower, tempUpper, marginError, noIter);
  } else {
    secant.noIterSecant(terms, coefficients, degree, tempLower, tempUpper, noIter);
  }
   ...
   if (upperFx.multiply(lowerFx).compareTo(new BigDecimal("0")) > 0) {
     JOptionPane.showMessageDialog(InitialBoundsSecant.this, "THIS FUNCTION DOESN'T PASS THE ROOT","Error",JOptionPane.ERROR_MESSAGE);
   ...
   JOptionPane.showMessageDialog(InitialBoundsSecant.this, "You can use the following Upper and Lower Limit:\n" + message);
