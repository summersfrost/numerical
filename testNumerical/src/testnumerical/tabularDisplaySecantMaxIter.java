 ...
 * @author Frost
 */
public class tabularDisplaySecantMaxIter extends javax.swing.JFrame {
/**
...
int middleSize;
public tabularDisplaySecantMaxIter(String terms, List < BigDecimal > lowerBound, List < BigDecimal > upperBound, List < BigDecimal > fxLowerBound, List < BigDecimal > fxUpperBound, List < BigDecimal > middleBound, List < BigDecimal > fxMiddleBound) {
        this.terms = terms;
        ...
        for (int i = 0; i < fxMiddleBound.size(); i++) {
            ...
            model.addRow(row);
            if (i > 0) {
        BigDecimal previousXm = middleBound.get(i - 1).setScale(10, RoundingMode.HALF_UP).stripTrailingZeros();
        // Check if the current result is the same as the previous result
        if (Xm.compareTo(previousXm) == 0) {
            break; // Exit the loop
        }
    }

        }
        jLabel1.setText("Therefore, x= " + middleBound.get(middleBound.size() - 1).setScale(10, RoundingMode.HALF_UP).stripTrailingZeros() + " obtained from the value of the root is accurate to the nearest hundreths after the " + middleBound.size() + " iteration.");
        setVisible(true);
    ...
    private void initComponents() {
        ...
        jLabel6.setText("Secant Method Table");
