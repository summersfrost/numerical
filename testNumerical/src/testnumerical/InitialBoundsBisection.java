...
//Check InitialBounds class since it consist of almost identical
public class InitialBoundsBisection extends javax.swing.JFrame {
    ...
    public InitialBoundsBisection(String terms, BigDecimal[] coefficients, int[] degree) {
        this.terms = terms;
    ...
    private void initComponents() {
        ...
        jLabel1.setText("INITIAL BOUNDS (BISECTION METHOD)");
  ...
  try {
    ...
    JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Upper and Lower must be real numbers.","Error",JOptionPane.ERROR_MESSAGE);
    return;
  }

  // Check that the upper and lower values are not equal
  if (upper.equals(lower)) {
    JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Upper and Lower cannot be equal.","Error",JOptionPane.ERROR_MESSAGE);
    return;
  }
  ...
  /*  BigDecimal three = new BigDecimal("3");
    if (upper.subtract(lower).abs().compareTo(three) > 0) {
        JOptionPane.showMessageDialog(this, "Upper and Lower cannot be more than 3 units apart.");
        return;
    }
    */
  ...
  if (upperFx.multiply(lowerFx).compareTo(new BigDecimal("0")) > 0) {
    JOptionPane.showMessageDialog(InitialBoundsBisection.this, "LOWER AND UPPER F(X) HAVE THE SAME SIGN","Error",JOptionPane.ERROR_MESSAGE);
    return;
  }
  this.upper = upper;
  this.lower = lower;
  ...
  if (marginErrorRadio) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Margin error must be a real number.","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    if (marginErrorValue.compareTo(BigDecimal.ZERO) <= 0 || marginErrorValue.compareTo(BigDecimal.ONE) >= 0) {
      JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Margin error must be between 0 and 1 (exclusive).","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    try {
      ...
      JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Number of iterations must be a positive integer.","Error",JOptionPane.ERROR_MESSAGE);
      return;
    }
    if (noIter <= 0) {
      JOptionPane.showMessageDialog(InitialBoundsBisection.this, "Number of iterations must be a positive integer.","Error",JOptionPane.ERROR_MESSAGE);
  ...
  CalcBisect bisect = new CalcBisect();
  if (modeIterRegula == 1) {
    bisect.errorBisect(terms, coefficients, degree, tempLower, tempUpper, marginError, noIter);
  } else {
    bisect.noIterBisect(terms, coefficients, degree, tempLower, tempUpper, noIter);
  }
  ...
  if (upperFx.multiply(lowerFx).compareTo(new BigDecimal("0")) > 0) {
      JOptionPane.showMessageDialog(InitialBoundsBisection.this, "THIS FUNCTION DOESN'T PASS THE ROOT","Error",JOptionPane.ERROR_MESSAGE);
  ...
  JOptionPane.showMessageDialog(InitialBoundsBisection.this, "You can use the following Upper and Lower Limit:\n" + message,"Error",JOptionPane.ERROR_MESSAGE);
